...
TELEGRAM_BOT_TOKEN = '7395213244:AAFvcf16F84HAHi8O97DTGZrdZU8VhSwA3Q'
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*🔥 Welcome to the Vip DDOS  🔥*\n\n"
        "*Use /attack <ip> <port> <duration>*\n"
        "*Buy Access From - @Manager_149⚔️💥*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️You need to be approved to use this bot⚠️*", parse_mode='Markdown')
        return

    # Check if the user already has an ongoing attack
    if user_attack_status.get(user_id, False):
        await context.bot.send_message(chat_id=chat_id, text="*⚠️You already have an attack in progress. Please wait for it to finish⚠️*", parse_mode='Markdown')
        return
